...
#define ERROR(format, ...) do{PRINTFUNC(NVP_PRINT_FD, "\033[01;33mNVP_ERROR\e[m (pid %i): " format, getpid(), ##__VA_ARGS__); fflush(NVP_PRINT_FD); _nv_error_count++; if(SPIN_ON_ERROR){ _nvp_debug_handoff(); } }while(0)
...
int _nv_error_count;
	...
	#define DEBUG(format, ...)   do{char loc; PRINTFUNC(NVP_PRINT_FD, "NVP_DEBUG (PID %i SP %p): "   format, getpid(), &loc, ##__VA_ARGS__); fflush(NVP_PRINT_FD); } while(0)
        #define WARNING(format, ...) do{PRINTFUNC(NVP_PRINT_FD, "NVP_WARNING (PID %i): " format, getpid(), ##__VA_ARGS__); fflush(NVP_PRINT_FD); } while(0)
	#define DEBUG_P(format, ...) do{PRINTFUNC(NVP_PRINT_FD,             format, ##__VA_ARGS__); fflush(NVP_PRINT_FD); } while(0)
#else
